...
- (IBAction)clearBtnClick:(id)sender {
    ...
    // Clear all cache.
    // 清空所有缓存
     [[CYVideoPlayerCache sharedCache] clearDiskOnCompletion:^{
...
- (void)calculateCacheMes{
    ...
    // 计算缓存大小
    [[CYVideoPlayerCache sharedCache] calculateSizeWithCompletionBlock:^(NSUInteger fileCount, NSUInteger totalSize) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (!strongSelf) return;
